...
## 6. プレイヤーアクション
### 注文タイプ (Order Types)
リアリティを持たせるため、多彩な注文方法を用意します。
*   **Market (成行)**: 即座に約定させる。スリッページのリスクあり。
*   **Limit (指値)**: 指定価格で待つ。流動性供給者（Maker）として手数料リベートを得られる（予定）。
*   **Stop / Stop Limit**: 損切りやブレイクアウト狙い。
*   **Trailing Stop**: 価格に合わせて逆指値ラインを自動追従させる。

### ショートカットキー操作
*   マウス操作だけでなく、キーボードのみで注文を発注・取消できる「ホットキー」機能を実装（プロゲーマーのような操作感）。
    *   例: `Ctrl + B`: 直近のBest Askを買う, `Esc`: 全注文キャンセル
...
*   **報酬**:
    *   **Persistent Rewards (永続報酬)**: 次のシーズンに引き継げる要素。
        *   **称号 (Titles)**: "Wolf of Paper Street", "Market Wizard" など。
        *   **スキン**: ターミナル画面のカラーテーマ、チャートのスタイル、アバターアイコン。
    *   **Skill Tree (Rogue-lite要素)**: 上位プレイヤーは、次シーズンのステータス（手数料割引、情報取得速度など）を微強化。
