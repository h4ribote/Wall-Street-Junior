...
#### 長期保有ボーナス (Long-term Holding Bonus)
プレイヤーの長期投資を推奨するため、同一銘柄の保有期間に応じて配当利回りにボーナスを付与します。

*   **保有期間の計算**: 加重平均取得日時（Weighted Average Acquisition Time）を使用します。
    *   追加購入時: `(現在の数量 × 現在の平均取得日時 + 購入数量 × 現在時刻) / (現在の数量 + 購入数量)`
    *   売却時: 平均取得日時は変化しません。
*   **ボーナス階層**:
    *   **< 14日 (1 Quarter)**: ボーナスなし (Base Yield)
    *   **14日 ～ 28日**: +10% Bonus
    *   **28日 ～ 42日**: +20% Bonus
    *   **> 42日**: +30% Bonus (Max)
