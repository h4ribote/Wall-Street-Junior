...
### 金利計算式 (Jump Rate Model)

在庫の減少（需要過多）を金利に反映させるため、DeFiレンディングプロトコル（Compound/Aave等）で採用されている **Jump Rate Model (Kink Model)** を導入します。
利用率が一定の閾値（Kink Point）を超えるまでは緩やかに上昇し、超えると急激に金利が跳ね上がる（Jumpする）仕組みです。

#### パラメータ定義

| パラメータ | 記号 | 設定値 (日次) | 説明 |
| --- | --- | --- | --- |
| **Base Rate** | $R_{base}$ | **0.00%** | 利用率0%の時の最低金利。 |
| **Multiplier** | $R_{slope1}$ | **0.05%** | 通常時の金利上昇係数。 |
| **Jump Multiplier** | $R_{slope2}$ | **1.00%** | 枯渇時のペナルティ金利係数。 |
| **Kink Point** | $U_{optimal}$ | **80% (0.80)** | 金利が跳ね上がる利用率の閾値。 |

#### 計算式

1. **通常時 ($U \le U_{optimal}$)**:
   $$R = R_{base} + R_{slope1} \times \frac{U}{U_{optimal}}$$

2. **枯渇時 ($U > U_{optimal}$)**:
   $$R = R_{base} + R_{slope1} + R_{slope2} \times (U - U_{optimal})$$

#### 具体例

*   **利用率 50%**: $0.00\% + 0.05\% \times (0.50 / 0.80) \approx 0.031\% / day$
*   **利用率 80%**: $0.00\% + 0.05\% = 0.050\% / day$
*   **利用率 90%**: $0.05\% + 1.00\% \times (0.90 - 0.80) = 0.05\% + 0.10\% = 0.150\% / day$
*   **利用率 100%**: $0.05\% + 1.00\% \times (1.00 - 0.80) = 0.05\% + 0.20\% = 0.250\% / day$

*   **徴収タイミング**: **2時間ごと**にポジションから徴収されます。
