...
*   **挙動**:
    ...
    *   **スプレッド計算 (Spread Calculation)**: ボラティリティと板の厚みに基づいて動的に決定されます。
        $$Spread = Base\ Spread \times (1 + \alpha \times \frac{Volatility}{Liquidity\ Depth})$$
        *   **Liquidity Depth**: 板に存在する注文総額。板が薄い（Depthが小さい）ほどスプレッドは拡大します。
        *   これにより、流動性の低い銘柄（Small Cap）での取引コストが増加するリスクを再現します。
...
*   **挙動**:
    ...
    *   **マーケットインパクト (Market Impact / Slippage)**:
        大口注文が価格を動かす幅を、以下のモデルで推定し、執行戦略（分割サイズ）を決定します。
        $$Impact(\%) = \gamma \times \sigma \times \sqrt{\frac{Order\ Size}{Daily\ Volume}}$$
        *   $\gamma$: 市場固有の係数
        *   $\sigma$: ボラティリティ
        *   流動性の低い銘柄（Small Cap）での大口取引は、自身の注文で不利な価格で約定するリスクが高まるため、より慎重に（時間をかけて）執行します。
...
*   **挙動**:
    ...
    *   **Creation Arbitrage (Premium時)**:
        *   Index価格 > NAV の場合。
        *   構成銘柄を市場で買い集める（異なる通貨での購入が必要な場合は、即座にFX市場で両替を行う）。
        *   現物バスケットをシステムに拠出し、Indexユニットを発行（Creation）。
        *   受け取ったIndexを市場で売却し、差益を得る。
    *   **Redemption Arbitrage (Discount時)**:
        *   Index価格 < NAV の場合。
        *   市場でIndexを購入する。
        *   Indexユニットをシステムに返還し、現物バスケットを受け取る（Redemption）。
        *   受け取った構成銘柄を市場で売却する（必要に応じてARCに両替）。
    *   **リスク要因**:
        *   **為替リスク**: 現物売買から両替完了までの間に為替レートが変動するリスク。
        *   **Execution Risk**: 多数の銘柄を同時に売買する必要があるため、一部の注文が約定しなかったり、予想以上のスリッページが発生するリスク（Legging Risk）。
